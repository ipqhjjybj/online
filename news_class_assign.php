<?php
	...
	if(!isset($_GET['news_class_id'])){
		...
		$id = $_GET['news_class_id'];
		$mysql = new OnlineSqlNew();
		$result = $mysql->select("news_class","id",$id);
...
<div id = "out" align="center">
    ...
    <form action="function/news_class_assign.php?<?php echo "news_class_id=".$row->id;?>" method="post" onSubmit="return check_news()">
    <table width="883" border="0">
      <tr>
        <td width="37">名称</td>
        <td colspan="2"><textarea name="title" id= "title" type="text"><?php echo $row->class_name;?></textarea></td>
