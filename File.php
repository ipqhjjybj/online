...
$sort=12;
$f_type=strtolower("swf,jpg,rar,zip,7z,iso,gif,png");//设置可上传的文件类型 
...
[$key], $uploadfile))){ 
				...
				//以下为添加到数据库的上传信息
				$upload_arr = array("null",$mysql->get_id_by_user_name($_SESSION['username']),$f_name,$uploadfile,date("Y-m-d"),date("H:i:s"));
				if(!$mysql->insert("upload",$upload_arr)){
					$_msg .= $f_name.'添加入数据库失败\n';	
				}
            	//
			} 
			else{
